...
control.vhd: control.psm
	picoasm -i control.psm -t ROM_form.vhd -m control

$(BUILD_DIR)/$(PROJECT).ngc: control.vhd kcpsm6.vhd $(PROJECT).vhd $(PROJECT).prj | $(BUILD_DIR)
	@cd $(BUILD_DIR); \
...
clean:
	rm -rf $(BUILD_DIR) control.vhd control.log
